...
const BookingManManagement = () => {
  ...
  function generateSeatLayout(totalSeats = 36) {
    ...
    for (let section = 0; section < 2; section++) {
      ...
      for (let row = 0; row < 6; row++) {
        ...
        const leftSeatNumber = section * 18 + row + 1
        const leftOccupied = Math.random() > 0.7
        const leftBookedByWomen = Math.random() > 0.8
        ...
        rowSeats.push({
          ...
          selected: false,
          isSingle: true,
          bookedByWomen: leftOccupied && leftBookedByWomen,
        ...
        if (section === 0) { // Lower berth
          // Specific combinations: 1,2; 5,6; 9,10; 13,14; 17,18; 21,22
          ...
          // Upper berth combinations: 3,4; 7,8; 11,12; 15,16; 19,20; 23,24
        ...
        const right1BookedByWomen = Math.random() > 0.8
        const right2BookedByWomen = Math.random() > 0.8
        ...
        rowSeats.push({
          ...
          bookedByWomen: right1Occupied && right1BookedByWomen,
          section: section === 0 ? 'lower' : 'upper',
          row: row + 1
        })
        ...
        rowSeats.push({
          ...
          bookedByWomen: right2Occupied && right2BookedByWomen,
          section: section === 0 ? 'lower' : 'upper',
          row: row + 1
        })
  ...
  const handleSeatClick = (seatNumber) => {
    ...
    const seat = selectedBus.seatLayout.flat().find(s => s.number === seatNumber)
    if (!seat || seat.occupied) return
    
    // Calculate bus occupancy percentage
    const totalSeats = selectedBus.seatLayout.flat().length
    const occupiedSeats = selectedBus.seatLayout.flat().filter(s => s.occupied).length
    const occupancyPercentage = (occupiedSeats / totalSeats) * 100
    
    // For right side double seats, handle pair selection/deselection
    if (seat.side === 'right' && !seat.isSingle) {
      const pairSeat = selectedBus.seatLayout.flat().find(s => s.number === seat.pairSeat)
      ...
      if (occupancyPercentage < 70) {
        ...
        if (selectedSeats.includes(seatNumber)) {
          ...
          setSelectedSeats(prev => prev.filter(num => num !== seatNumber && num !== seat.pairSeat))
        } else {
          // Selecting: add both seats to selection
          if (pairSeat && !pairSeat.occupied) {
            setSelectedSeats(prev => [...prev, seatNumber, seat.pairSeat])
          } else {
            alert('Both seats in this pair must be available to book together')
            return
          }
        }
      } else {
                    ...
                    <div className="card">
                      ...
                      <div className="flex justify-center mb-6">
                        <div className="flex flex-wrap gap-4 text-sm">
                          ...
                          <div className="flex items-center">
                            <div className="w-6 h-6 rounded border-2 border-blue-300 bg-blue-100 mr-2 flex items-center justify-center">
                                  ...
                                  {section.map((row, rowIndex) => (
                                    <div key={rowIndex} className="flex justify-center space-x-8">
                                      ...
                                      <div className="flex flex-col">
                                        <button
                                          key={row[0].number}
                                          onClick={() => !row[0].occupied && handleSeatClick(row[0].number)}
                                          className={`w-8 h-16 rounded border-2 text-xs font-medium flex items-center justify-center ${
                                            row[0].occupied 
                                              ? row[0].bookedByWomen
                                      ...
                                      <div className="flex space-x-1">
                                        <button
                                          key={row[1].number}
                                          onClick={() => !row[1].occupied && handleSeatClick(row[1].number)}
                                          className={`w-8 h-16 rounded border-2 text-xs font-medium flex items-center justify-center ${
                                            row[1].occupied 
                                              ? row[1].bookedByWomen
                                                ? 'border-pink-300 bg-pink-100 text-pink-600 cursor-not-allowed'
                                        ...
                                        <button
                                          key={row[2].number}
                                          onClick={() => !row[2].occupied && handleSeatClick(row[2].number)}
                                          className={`w-8 h-16 rounded border-2 text-xs font-medium flex items-center justify-center ${
                                            row[2].occupied 
                                              ? row[2].bookedByWomen
                                                ? 'border-pink-300 bg-pink-100 text-pink-600 cursor-not-allowed'
                      ...
                      <div className="mt-6 text-center text-sm text-gray-600">
                        ...
                        <div className="mt-2 text-xs">
                          Bus Occupancy: {selectedBus ? Math.round((selectedBus.seatLayout.flat().filter(s => s.occupied).length / selectedBus.seatLayout.flat().length) * 100) : 0}%
                          {selectedBus && (selectedBus.seatLayout.flat().filter(s => s.occupied).length / selectedBus.seatLayout.flat().length) * 100 < 70 && (
