          ...
          <div className="flex justify-between items-center h-10.5">
            ...
            <div className="flex-1 text-center hidden md:block lg:block">
              {user && user.role !== 'MASTER_ADMIN' && user.company && (
                <h1 className="text-sm lg:text-base xl:text-lg font-semibold text-gray-600 truncate px-2">
                  {user.company}
                </h1>
              )}
            </div>
            ...
            <div className="hidden sm:flex items-center space-x-2 flex-shrink-0">
              {user ? (
                <>
                  ...
                  <div className="hidden md:block px-2 py-1 rounded-full text-xs font-medium bg-white text-gray-600 border border-hover">
                ...
                <Link
                  to="/login"
                  className="flex items-center space-x-1 px-3 py-1.5 rounded-md text-sm font-medium text-gray-600 hover:text-white hover:bg-hover/50 transition-all duration-200"
                >
            ...
            <div className="sm:hidden flex items-center space-x-2">
              {user && (
                <div className="px-2 py-1 rounded-full text-xs font-medium bg-white text-gray-600 border border-hover">
                  {user.role === 'MASTER_ADMIN' ? 'Master Admin' :
                   user.role === 'BUS_OWNER' ? 'Bus Owner' :
                   user.role === 'BUS_ADMIN' ? 'Bus Admin' :
                   user.role === 'BOOKING_MAN' ? 'Booking Manager' :
              ...
              <button
                ...
                className="p-1.5 rounded-md text-gray-600 hover:text-white hover:bg-hover/50 transition-all duration-200"
            ...
            <div className="sm:hidden border-t border-hover bg-white shadow-sm">
              <div className="px-3 py-2">
                {user ? (
                  ...
                  <Link
                    to="/login"
                    className="flex items-center space-x-2 px-3 py-1.5 rounded-md text-sm font-medium text-gray-600 hover:text-gray-600 hover:bg-gray-100/50 transition-all duration-200"
